...
class EnvBall:
    def __init__(self, 
        ...
        self.depth_cam = Depth_Camera("depth_cam", self.supervisor, self.arm.timestep)
        # ------------------------------------------------------------------------------
        ...
        print(f"observation_shape:\t{self.observation_space.shape}")
    ...
    def reset(self, seed, is_random=True, is_add_target_noise=False, is_candidate=True):
        # ------------------------------------------------------------------------------
        # init env seed
        ...
        if is_random:
            self.ball.random_choose_one_position(is_add_noise=is_add_target_noise, is_candidate=is_candidate)
        else:
            self.ball.new_position = self.ball.position_candididate[1]
        # ------------------------------------------------------------------------------
        # set target object         
    ...
    def get_observation(self):
        ...
        self.observation.interact_object = self.ball.position.copy()
        # ------------------------------------------------------------------------------
    ...
    def execute_action(self, action):
        ...
        if self.is_going_to_target:
            self.arm.movP(self.observation.interact_object)
...
class Depth_Camera:
    ...
    def show_frame(self):
        cv2.imshow("depth_cam_node image", self.get_frame())
        cv2.waitKey(1) 
        return None
...
class Object:
    ...
    def random_choose_one_position(self, is_add_noise=False, is_candidate=True):
        # --------------------------------------------------
        ...
        # --------------------------------------------------
        # add noise into new position
        if is_add_noise:
            print("is_add_noise", is_add_noise)
            # 指定均值和標準差
            mu = 0  # 均值
            sigma = 0.05  # 標準差
            num_samples = 3  # 生成的隨機數個數

            # 生成符合正態分佈的隨機數
            random_numbers = np.random.normal(mu, sigma, num_samples)
            self.new_position = list(self.new_position + random_numbers)
        # --------------------------------------------------
